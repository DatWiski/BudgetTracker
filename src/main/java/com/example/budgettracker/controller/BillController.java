...
public class BillController {
  ...
  public ResponseEntity<BillResponse> addBill(
      @RequestBody(description = "Bill details", required = true)
          @Valid
  ...
  @Operation(
      ...
      description = "Updates an existing bill for the authenticated user")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<BillResponse> updateBill(
      ...
      @RequestBody(description = "Updated bill details", required = true)
          @Valid
          @org.springframework.web.bind.annotation.RequestBody
          BillRequest billRequest,
      @Parameter(hidden = true) AppUser appUser) {
  ...
  @Operation(
      summary = "List bills",
      description = "Retrieves a paginated list of bills for the authenticated user")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<Page<BillResponse>> listBills(
      @Parameter(hidden = true) AppUser appUser,
      @Parameter(description = "Pagination parameters") @PageableDefault(size = 20, sort = "name")
          Pageable pageable) {
    Page<BillResponse> response =
        billService
            .getBillsForUser(appUser, pageable)
