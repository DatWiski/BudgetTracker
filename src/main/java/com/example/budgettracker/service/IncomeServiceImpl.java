...
public class IncomeServiceImpl implements IncomeService {
  ...
  public Income updateIncomeForUser(Long incomeId, IncomeRequest request, AppUser user) {
    Income income =
        incomeRepository
            .findByIdAndAppUser(incomeId, user)
            .orElseThrow(
                () -> new IncomeNotFoundException("Income not found with ID: " + incomeId));
  ...
  public List<Income> getIncomeForUser(AppUser user) {
    return incomeRepository
        .findByAppUserOrderByIncomeDateDesc(user, Pageable.unpaged())
        .getContent();
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Income> getIncomeForUserAndDateRange(
      AppUser user, LocalDate startDate, LocalDate endDate) {
    return incomeRepository.findByAppUserAndIncomeDateBetweenOrderByIncomeDateDesc(
